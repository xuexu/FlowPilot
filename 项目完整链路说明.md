...
Duck 邮箱生成现在分为两种模式，配置字段为 `duckEmailGenerationMode`：

1. `page` 是默认模式，继续打开 DuckDuckGo Email Protection 页面，并通过现有 `FETCH_DUCK_EMAIL` 内容脚本读取或生成 `@duck.com` 地址。
2. `token` 是 DDG Token 直连模式，后台优先读取持久配置 `duckDdgToken`，直接调用 `https://quack.duckduckgo.com/api/email/addresses` 生成 `@duck.com` 地址，不再依赖每次打开页面。
3. 当 `token` 模式下 `duckDdgToken` 为空时，后台会打开 DuckDuckGo Email Protection 页面，通过 `chrome.webRequest.onBeforeSendHeaders` 监听 `quack.duckduckgo.com/api/email/addresses` 请求头，并触发一次 `FETCH_DUCK_EMAIL` 让页面产生真实 DDG API 请求；捕获到 `Authorization: Bearer ...` 后会自动回填并保存 `duckDdgToken`，再继续用 Token 直连生成邮箱。
4. 自动捕获 Token 需要用户已登录 DuckDuckGo 且已启用 Email Protection；未登录、未启用、监听能力缺失、请求失败或超时都会用中文错误停止当前邮箱生成，不会自动回退到 `page` 模式。
